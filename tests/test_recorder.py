...
@pytest.fixture
def recorder(simple_circuit: QuantumCircuit) -> StateEvolutionRecorder:
...
def test_initialization_with_custom_state(simple_circuit: QuantumCircuit):
...
def test_initialization_with_custom_parameters(simple_circuit: QuantumCircuit):
    """Test initialization with custom parameters."""
    recorder = StateEvolutionRecorder(
        simple_circuit, figsize=(8, 8), dpi=150, num_cols=3, select=[0], style={"name": "textbook"}
    )
    assert recorder._frame_renderer._fig is not None
...
def test_evolve_without_intermediate_steps(recorder: StateEvolutionRecorder):
...
def test_evolve_with_intermediate_steps(recorder: StateEvolutionRecorder):
...
def test_record_creates_file(recorder: StateEvolutionRecorder, tmp_path: Path):
...
def test_invalid_circuit():
    ...
    with pytest.raises(Exception):
        StateEvolutionRecorder(None)  # type: ignore[reportArgumentType]
